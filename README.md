...
The compressed experiment files can be found [here](https://github.com/iamit87/crispector/raw/master/example/EMX1_11_sites_50k_reads.zip), and it contain the following files: 
...
Usage with demultiplexed input (FASTQ file for each locus site) is identical to default usage with the followoing exception - Full paths to the FASTQ files are given in the experiment config, instead of command-line arguments.  The paths sould be suplied for each target locus in the following experiment configuartion collumns:
...
```
**Exmaple:**
You can download data and configuration for EMX1 experiemt (performed with [rhAmpSeq](https://eu.idtdna.com/pages/products/next-generation-sequencing/amplicon-sequencing?utm_source=google&utm_medium=cpc&utm_campaign=ga_rhampseq&utm_content=ad_group_rhampseq&gclid=Cj0KCQjw3qzzBRDnARIsAECmryqo5fO62fqk95a4PfkqES-9G07br5kdtTpjJInnYFjqYw2OxYI2gRwaAmTQEALw_wcB)) . Experiment was designed with one on-target site and 10 off-target sites. The FASTQ files contain the first 5,000 reads for each target locus. Loci Sites were demultiplexed to seperated FASTQ files using bowtie2.   The compressed experiment files can be found [here](https://github.com/iamit87/crispector/raw/master/example/EMX1_11_sites_demultiplexed_input_500k_reads.zip). Make sure you change "PATH_TO_DIRECTORY" in EMX1_config.csv to your own local directory path. 
```
...
There are two main parameter types that can be tuned in the configuration file:
1. [Alignment](###Alignment)
2. [NHEJ inference](###NHEJ-inference)
